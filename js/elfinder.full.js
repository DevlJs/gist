	...
	function(ql) {
		...
		preview.bind('update', function(e) {
			...
			if (ql.support.video[type]) {
				...
				ql.hideinfo();
                console.log(11);
				node = $('<video class="elfinder-quicklook-preview-video" controls crossorigin preload="auto" autobuffer><source src="'+ql.fm.openUrl(file.hash)+'" type="video/mp4"/></video>').appendTo(preview);
				autoplay && node[0].play();
